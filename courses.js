...
//this counts the amount of courses
let amountOfCourses = courses.length; 

console.log(amountOfCourses);

//use the for loop to display course titles
...
//filter courses - AK-47
//courses.filter - filters through all the courses and then you give it a name that =>(goes) to the namegiven.what your are looking for === specific course
let courseTitle = courses.filter(course => course.CourseCode === "AK-47");
let coursePrice = courseTitle.map(course => (course.Price));
console.log(`The AK-47 course cost ${coursePrice}`);
